    ...
    useEffect(() => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          const { error: cursoModuloError } = await supabase.from('curso_modulo').insert([
            {
        ...
        <div className='rounded-lg'>
            {showSuccessMessage && (
              <div variant="faded" className="absolute top-2 right-2 bg-green-800 text-white py-2 px-4 rounded">
                <RiThumbUpFill className="mr-2 text-white"/>Mòdul creat amb èxit
              </div>
            )}
                <Button  className="" onClick={() => setAddModulSelected(!addModulSelected)}>Crear mòdul  {addModulSelected ? <RiCloseFill className='text-red-800'/> : <RiAddFill />}</Button>
                ...
                <Select 
                  ...
                  onChange={(e)=>handleInputChange(e.target.value,'curso_id')}
                  color="secondary"
                  label="Selecciona un curs"
                >
                  {cursos.map(curso => (
                    ...
                    <div className='p-8 flex flex-col gap-4 '>
                      ...
                      {["video", "acciona", "quiz", "act2", "act3", "material", "examen"].map((element) => (
                        <div className="module-element justify-between flex items-center" key={element}>
                          <Checkbox 
                            name={`have_${element}`} 
                            onValueChange={(value) => handleCheckboxChange(`have_${element}`, value)} 
                            isSelected={moduloData[`have_${element}`]}
                          />
                          <span>{element.charAt(0).toUpperCase() + element.slice(1)}</span>
                          <select
                            className="ms-3 w-[20%] flex text-center border rounded-sm border-gray-800" 
                            name={`${element}_cantidad`}
                            value={moduloData[`have_${element}`] ? moduloData[`${element}_cantidad`] : 0}
                    ...
                    <Button className="bg-green-200 w-full" type="submit">Afegir</Button>
